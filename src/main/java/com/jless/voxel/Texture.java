...
public final class Texture {
  ...
  public static Texture load(String filePath, boolean flipY) {
  ...
  public static Texture loadResource(String cp, boolean flipY) {
    if(!cp.startsWith("/")) {
    ...
    try {
      ...
      Texture tex = uploadRGBA(pixels, w.get(0), h.get(0));
      STBImage.stbi_image_free(pixels);
      return tex;
    } catch(IOException e) {
  ...
  private static Texture uploadRGBA(ByteBuffer pixels, int w, int h) {
    int texID = glGenTextures();
    glBindTexture(GL_TEXTURE_2D, texID);
